...
특정 회차를 오래 간직하고 싶을 때 만드는 **자기완결형 HTML**입니다.
- 파일명: `collection-<날짜>-<주제>.html` (예: `collection-2026-08-12-umi.html`)
- 대표 그림을 **파일 안에 base64로 내장**하므로 `images/` 폴더 없이 그 파일 하나만 있으면 열립니다.
  원본이 화면 표시 크기(최대 높이 440px)에 비해 과도하게 크므로 가로 1600px·JPEG 85로 줄여 넣습니다.
  (참고 실적: 19.4MB → 1.4MB, 23.5MB → 2.6MB)
- 하단 "지난 브리핑" 목록과 상단 보관함 링크는 제거합니다(단일 파일이라 링크가 깨지므로).
